...
function show_thermometer(container, currentValue) {
    ...
    var domain = [step * Math.floor(minValue / step), step * Math.ceil(maxValue / step)];

    // D3 scale object
    var scale = d3.scaleLinear().range([bulb_cy - bulbRadius + PADDING, top_cy]).domain(domain);


    var tubeFill_bottom = bulb_cy, tubeFill_top = scale(currentValue);
    ...
    var textCol = "rgb(230, 0, 0)", textOffset = -4;
